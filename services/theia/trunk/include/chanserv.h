 ...
 *
 * $Id: chanserv.h,v 1.7 2001/06/14 08:21:50 kreator Exp $
 */
...
#define CS_SECUREOPS    0x00000008 /* only aop/sop/contacts can be opped */
...
struct ChanAccess
{
  ...
  time_t last_used; /* last time the person joined the channel while identified */
};

struct AutoKick
{
  struct AutoKick *next;
  char *hostmask; /* mask to autoremove */
  char *reason;   /* reason for autoremove */
};
...
struct ChanInfo
{
  ...
  char *contact;                /* contact nick (must be registered) */
  time_t last_contact_active;   /* last time the contact joined/left */
  char *alternate;              /* alternate contact nick (must be registered) */
  time_t last_alternate_active; /* last time the contact joined/left */
  char *password;               /* contact password */
  char *topic;                  /* NULL if no topic lock */
  time_t topic_ts;
  long limit;                   /* 0 if no limit */
  char *key;                    /* NULL if no key */
  char *forward;                /* NULL if no forward target */
  char *throttle;               /* NULL if no throttle limits */
  char *dline;			/* NULL if no autodline limits */
  int modes_on,                 /* modes to enforce */
      modes_off;                /* modes to enforce off */
  struct ChanAccess *access;    /* access list */
  int akickcnt;                 /* number of autoremoves */
  struct AutoKick *akick;       /* autoremove list */

  char *entrymsg;               /* msg to send to users upon entry to channel */
  char *email;                  /* email address of channel */
  char *url;                    /* url of channel */

  /* list of users who have contact access */
  struct f_users
  {
    struct f_users *next;
    struct Luser *lptr;
...
void cs_CheckTopic(char *who, char *channel, char *, time_t);
int cs_ShouldBeOnChan(struct ChanInfo *cptr);
void cs_RejoinChannels();
void MakeContact(struct ChanInfo *cptr);
...
void RemContact(struct Luser *lptr, struct ChanInfo *cptr);
