 ...
 *
 * $Id: nickserv.h,v 1.3 2001/05/28 23:46:32 kreator Exp $
 */
...
#define NS_CLOAK	0x00400000 /* cloak upon identify */
#define NS_UNFILTERED	0x00800000 /* turn on 'unfiltered' upon identify */
#define NS_EXCHANS	0x01000000 /* give user +u on connect(extended channel limit) */
...
struct NickInfo
{
  ...
  char *cloak;               /* string to sethost to */
...
#ifdef CHANNELSERVICES
  /*
   * List of channels for which this nickname is a contact
   */
  struct aChannelPtr *ContactChannels;
...
void AddContactChannelToNick(struct NickInfo **nptr,
                             struct ChanInfo *cptr);
void RemoveContactChannelFromNick(struct NickInfo **nptr,
                                  struct ChanInfo *cptr);
...
void collide(char *nick, struct Luser *source);
