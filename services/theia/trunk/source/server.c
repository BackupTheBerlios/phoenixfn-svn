...
static void s_snick(int ac, char **av);
...

{
  ...
  struct hostent *host;
#ifdef NICKSERVICES
  struct NickInfo *nptr, *newptr, *realptr=0;
#endif
  ...
  if (ac == 4)
  {
    ...
    /* Client changing case of nick, ignore. */
    if (!irccmp(who, av[2]))
    {
      /* Must be same length, so OK even without BLOCK_ALLOCATION */
      strncpy(lptr->nick, av[2], NICKLEN);
      return;
    }
    ...
    if (nptr)
    {
        ...
        if (newptr && IsLinked(nptr, newptr))
	  {
	    newptr->flags |= NS_IDENTIFIED;
	    toserv(":%s MODE %s +e\n", Me.name, newptr->nick);
            realptr = GetMaster(newptr);
	  }
        else
          {
	    toserv(":%s MODE %s -6u\n", Me.name, newnick);
            if ((realptr->flags & NS_CLOAK) && (realptr->cloak))
              mustremovecloak = 1;
          }
      }
    #else /* LINKED_NICKNAMES */
      toserv(":%s MODE %s -6u\n", Me.name, newnick);
    ...
    if (mustremovecloak)
      do_sethost(n_NickServ, lptr, lptr->orighost);
    ...
    if (realptr)
    {
      if ((realptr->cloak) && (realptr->flags & NS_CLOAK))
        es_add(oldnick, lptr->username, realptr->cloak, NULL, current_ts, 2);
    }
    else
    {
...
/*
s_snick()
  Process a SNICK statement (extension to nick)
  av[0] = "SNICK"
  av[1] = who
  av[2] = sign-on nick
  av[3] = spoofhost
...
*/
static void
s_snick(int ac, char **av)

{
  char *who, /* nick we're dealing with */
       *orighost; /* original host to set back */
  struct Luser *lptr;
  ...
  if (ac > 5)
  {
    if (irccmp(av[3], lptr->hostname))
      orighost = av[5]; /* User is not using spoofhost */
    else
      orighost = lptr->hostname; /* User is using spoofhost, assume I:line cloak */
    #ifdef BLOCK_ALLOCATION
      strcpy(lptr->orighost, orighost);
    #else
      MyFree(lptr->orighost);
      lptr->orighost = MyStrdup(orighost);
    #endif /* BLOCK_ALLOCATION */
  }
} /* s_snick() */
...
/*
s_kick()
  Process a KICK or REMOVE statement
  av[0] = who
  av[1] = "KICK" or "REMOVE"
  av[2] = channel
  av[3] = nickname
...

{
  ...
  if (lptr)
  {
    putlog(LOG3, "%s: %s!%s@%s %s %s [%s]",
      av[2],
      lptr->nick,
      lptr->username,
      lptr->hostname,
      !irccmp("REMOVE", av[1]) ? "removed" : "kicked",
      av[3],
      av[4] + 1);
    ...
    SendUmode(OPERUMODE_K,
      "*** %s: %s [%s] by %s!%s@%s (%s)",
      av[2],
      !irccmp("REMOVE", av[1]) ? "Remove" : "Kick",
      av[3],
      lptr->nick,
      lptr->username,
      lptr->hostname,
...

{
  ...
  /* Anything before the list of names, is part of the mode string */
  while (mcnt < ac && av[mcnt][0] != ':')
    {
      strcat(modes, " ");
  ...
  /* Sanity check: we should have something left after that */
  if (mcnt >= ac)
    return;
  ...
  if (ci)
  {
    ...
    for (ii = 0; ii < ncnt; ii++)
    {
      ...
      /*
       * Check if nick is on the AUTOREM list for chname
       */
...

{
  ...
  if (isoper)
  {
    toserv(":%s 313 %s %s :is a Network Service\n",
      Me.name,
      who,
      serviceptr->nick);
  }
...

{
  ...
  toserv(":%s 351 %s theia-%s. %s :dncrTS/v4\n",
    Me.name,
    who,
    hVersion,
    Me.name);
...
s_sethost( int ac, char **av )
{
  ...
  /* We shouldn't get this, but anyway */
  if (!strcmp(".dns", newhost))
    newhost = lptr->orighost;
