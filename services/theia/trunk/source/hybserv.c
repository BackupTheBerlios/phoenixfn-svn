 ...
 *
 * $Id: hybserv.c,v 1.3 2001/11/12 09:50:55 asuffield Exp $
 */
...
int main(int argc, char *argv[])
{
  ...
  if (pid != 0)
  {
    printf("theia: version %s\n", hVersion);
    printf("theia: pid %d\n", pid);
    printf("theia: running in background mode from %s\n", HPath);
    exit(0);
  }
		...
		if (len == -1)
		...
		else
		  {
			...
			*next = '\0';
			...
			p = next + 1; /* At most points to buf[511], since strchr() can never get past that \0 */
			next = strchr(p, '\n');
		      }
