 ...
 *
 * $Id: jupe.c,v 1.3 2001/11/12 09:50:55 asuffield Exp $
 */
  ...
  for (tempjupe = JupeList; tempjupe; tempjupe = tempjupe->next)
  {
    if (match(tempjupe->name, name))
    {
      if (tempjupe->isnick)
      {
        ...
        ircsprintf(sendstr, "NICK %s 1 %ld +i juped juped.com %s %lu :%s\n",
		   tempjupe->name,
#ifdef NICKSERVICES
		   (long) (lptr->nick_ts - 1),
#else
...

{
  ...
  for (tmpjupe = JupeList; tmpjupe; tmpjupe = tmpjupe->next)
  {
    ...
    {
      /* collide the nickname */
      ircsprintf(sendstr, "NICK %s 1 1 +i juped juped.com %s %lu :%s\n",
          tmpjupe->name, Me.name, 0xffffffffL, tmpjupe->reason ? tmpjupe->reason :
          "Jupitered Nickname");
      toserv(sendstr);
