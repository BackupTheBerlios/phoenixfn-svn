 ...
 *
 * $Id: hash.c,v 1.3 2001/11/12 09:50:55 asuffield Exp $
 */
...
/* Cope with both ~noident/ident and n=noident/i=ident -- jilles */
#define USERNAME_IS_IDENTD(n) ((n)[0] != '~' && \
		((n)[1] != '=' || ((n)[0] != 'n' && (n)[0] != 'N')))
...

{
   ...
   */
  if (!USERNAME_IS_IDENTD(lptr->username))
  {
    /* Yes this breaks with i=/n=; noone uses the broken clone detection
     * anyway */
