 ...
 *
 * $Id: nickserv.c,v 1.3 2001/11/12 09:50:55 asuffield Exp $
 */
...
/* Hack to disable REGISTER temporarily */
int RegisterDisabled = 0;
...
static void n_set_unfiltered(struct Luser *, int, char **);
#ifdef LINKED_NICKNAMES
...
static void n_setcloak(struct Luser *, int, char **);
static void n_noexpire(struct Luser *, int, char **);
static void n_exchans(struct Luser *, int, char **);
...
static struct Command nickcmds[] = {
  ...
  { "SETCLOAK", n_setcloak, LVL_USERADMIN },
  { "CLEARNOEXP", n_clearnoexp, LVL_ADMIN },
  { "FIXTS", n_fixts, LVL_ADMIN },
  { "EXTENDCHANS", n_exchans, LVL_ADMIN },
...
/*   { "OPER", n_set_oper, LVL_NONE }, */
  ...
  { "UNFILTERED", n_set_unfiltered, LVL_NONE },
...

{
  ...
  /*
   * Check if the command is for user admins only - if so,
   * check if they match an admin O: line, and are
   * currently registered with OperServ.
...
/*
AddContactChannelToNick()
 Each nick structure contains a list of channels for which they
are registered as a contact (ie: they created the channel). Add
such a channel to 'nptr's list
*/

void
AddContactChannelToNick(struct NickInfo **nptr, struct ChanInfo *cptr)

{
  ...
  tmp->cptr = cptr;
  tmp->next = (*nptr)->ContactChannels;
  (*nptr)->ContactChannels = tmp;

  ++(*nptr)->fccnt;
} /* AddContactChannelToNick() */

/*
 * RemoveContactChannelFromNick()
 *
 * Remove 'cptr' from 'nptr's contact channel list
*/

void RemoveContactChannelFromNick(struct NickInfo **nptr, struct ChanInfo
    *cptr)
{
  ...
  /* Iterate list of nick's contact channels */
  for (tmp = (*nptr)->ContactChannels; tmp; )
  {
    /* We have a match! */
    if (tmp->cptr == cptr)
    {
...

{
  ...
  while (nickptr->ContactChannels)
  {
    cptr = nickptr->ContactChannels->cptr;

    ftmp = nickptr->ContactChannels->next;
    MyFree(nickptr->ContactChannels);
    nickptr->ContactChannels = ftmp;
     ...
     * try to call RemoveContactChannelFromNick(). This would be very bad,
     * since this loop is modifying nickptr's ContactChannels list. Right
     * now, all calls to DeleteNick() remove nickptr from nicklist[]
     * beforehand, but, being as paranoid as I am, we'll set cptr->contact
     * to null here, so there is *NO* chance of it ever being used to
     * delete nickptr's ContactChannels list.
     */
    if (cptr->contact)
    {
      MyFree(cptr->contact);
    ...
    /*
     * If the channel has an alternate, change them to contact,
...

{
  ...
  nptr->password = MyStrdup(pwmake(newpass, nptr->nick));
...

{
  ...
  if ((knownhost) && (nptr->flags & NS_UNSECURE))
  {
    ...
    toserv(":%s MODE %s +%s%se\n", Me.name, lptr->nick,
      (nptr->flags & NS_UNFILTERED) ? "6" : "",
      (nptr->flags & NS_EXCHANS) ? "u" : "");
  }
  return 1;
} /* CheckNick() */
...
void
collide(char *nick, struct Luser *source)

{
  ...
  /*
   * Send a server colide since they're nice and quiet.
   */
  if (source != NULL)
    toserv("COLLIDE %s :RECOVER command used by %s\n%s",
      lptr->nick,
      source->nick,
      sendstr);
  else
    ...
    for (nptr = nicklist[ii]; nptr; nptr = nptr->next)
    {
      if (nptr->flags & NS_COLLIDE)
      {
        ...
        if ((lptr = FindClient(nptr->nick)))
        {
          if (unixtime >= nptr->collide_ts)
          {
             ...
             */
            putlog(LOG1, 
              "%s: colliding %s!%s@%s (not identified in time)",
              n_NickServ,
              lptr->nick,
              lptr->username,
              lptr->hostname);
      ...
      else if (NSReleaseTimeout && (nptr->flags & NS_RELEASE))
      {
        ...
        if ((lptr = FindClient(nptr->nick)))
        {
          ...
          {
            putlog(LOG1,
              "%s: Releasing nickname reservation pseudo-nick [%s]",
              n_NickServ,
              lptr->nick);
...
#ifdef CHANNELSERVICES
/* Transfer ContactChannels and AccessChannels from 'from' to 'to'
 * -- jilles */
static void TransferChannels(struct NickInfo *from, struct NickInfo *to)
{
	struct aChannelPtr *chan;
	struct AccessChannel *atmp;
	struct ChanInfo *ci;
	int changed = 0;

	/* Transfer ContactChannels */
	/*
	 * from should no longer have a ContactChannels list - add all of
	 * from's channels to to's channels. There's no point in
	 * reallocating - just assign to's pointer to from's
	 *
	 * ...or rather, add them.
	 */

	/* don't forget to update the contacts in the channels -- jilles */
	chan = from->ContactChannels;
	while (chan != NULL)
	{
		MyFree(chan->cptr->contact);
		chan->cptr->contact = MyStrdup(to->nick);
		chan = chan->next;
	}
	if (to->ContactChannels)
	{
		struct aChannelPtr *tmpchan = to->ContactChannels;
		while (tmpchan->next)
			tmpchan = tmpchan->next;
		tmpchan->next = from->ContactChannels;
		to->fccnt += from->fccnt;
	}
	else
	{
		to->ContactChannels = from->ContactChannels;
		to->fccnt = from->fccnt;
	}
	from->ContactChannels = NULL;
	from->fccnt = 0;

	/* Transfer AccessChannels */
	atmp = from->AccessChannels;
	while (atmp != NULL)
	{
		if (atmp->accessptr->nptr != from)
		{
			if (atmp->accessptr->nptr)
				putlog(LOG2, "WTF: While transferring AccessChannels from %s to %s, found access entry for %s on list (updated to %s)", from->nick, to->nick, atmp->accessptr->nptr->nick, to->nick);
			else
			{
				putlog(LOG2, "WTF: While transferring AccessChannels from %s to %s, found hostmask access entry for %s on list (moved to new list but not altered)", from->nick, to->nick, atmp->accessptr->hostmask);
				continue;
			}
		}
		atmp->accessptr->nptr = to;
		/* let's update successors here too, there's no separate list */
		ci = atmp->cptr;
		if (ci->alternate != NULL && !irccmp(ci->alternate, from->nick))
		{
			MyFree(ci->alternate);
			ci->alternate = MyStrdup(to->nick);
		}
		atmp = atmp->next;
	}
	atmp = to->AccessChannels;
	if (atmp != NULL)
	{
		while (atmp->next != NULL)
			atmp = atmp->next;
		atmp->next = from->AccessChannels;
		if (atmp->next != NULL)
			atmp->next->prev = atmp;
	}
	else
		to->AccessChannels = from->AccessChannels;
	from->AccessChannels = NULL;

	/* OK, now we added the two lists, couldn't there be some dups?
	 * -- jilles */
	do
	{
		changed = 0;
		...
		while (atmp != NULL)
		{
			if (RemoveDupAccess(atmp->cptr, to))
			{
				/* restart the loop to avoid bad mem refs */
				changed = 1;
				break;
			}
			atmp = atmp->next;
...
#ifdef CHANNELSERVICES
  TransferChannels(leafmaster, master);
#endif /* CHANNELSERVICES */

  /* Let's not overwrite cloaks here. */
  if (leafmaster->flags & NS_CLOAK && leafmaster->cloak != NULL &&
      master->cloak == NULL)
  {
    master->cloak = MyStrdup(leafmaster->cloak);
    master->flags |= NS_CLOAK;
  }
...
static int DeleteLink(struct NickInfo *nptr, int copyhosts)
{
  ...
  else /* nptr->master is NULL indicating this is master nick */
  {
    ...
    master->master = NULL;
    master->numlinks = nptr->numlinks;
    for (tmp = master->nextlink; tmp; tmp = tmp->nextlink)
      tmp->master = master;

    if (copyhosts)
      for (hptr = nptr->hosts; hptr; hptr = hptr->next)
        AddHostToNick(hptr->hostmask, master);

#ifdef CHANNELSERVICES
    TransferChannels(nptr, master);
#endif /* CHANNELSERVICES */

    if (nptr->flags & NS_CLOAK && nptr->cloak != NULL)
    {
      if (master->cloak != NULL)
        MyFree(master->cloak);
      master->cloak = MyStrdup(nptr->cloak);
      master->flags |= NS_CLOAK;
    }
  }

  /* no matter what happened, nptr is supposed to be master now. And alone. */
...

{
  ...
  if (!IsValidAdmin(lptr))
    if (RegisterDisabled)
      {
	notice(n_NickServ, lptr->nick,
	       "Registration has been temporarily suspended. We apologize for the inconvenience.");
	return;
      }
  ...
  /* Don't do this. Period. If it's meant to be NOEXPIRE, an admin can NOEXPIRE it */
/*   if (IsOperator(lptr)) */
/*     nptr->flags |= NS_OPERATOR; */
  ...
  notice(n_NickServ, lptr->nick,
    ...
    mask);
  notice(n_NickServ, lptr->nick,
    "Your password is [\002%s\002].  Please remember this for later use.",
    av[1]);
#ifdef FREENODE
  notice(n_NickServ, lptr->nick,
    "Freenode is a service of Peer-Directed Projects Center, an");
  notice(n_NickServ, lptr->nick,
    "IRS 501(c)(3) (tax-exempt) charitable and educational organization.");
  notice(n_NickServ, lptr->nick,
    "For frequently-asked questions about the network, please see the");
  notice(n_NickServ, lptr->nick,
    "FAQ page (http://freenode.net/faq.shtml).");
#endif
  toserv(":%s MODE %s +%s%se\n", Me.name, lptr->nick,
    (nptr->flags & NS_UNFILTERED) ? "6" : "",
    (nptr->flags & NS_EXCHANS) ? "u" : "");
...

{
  ...
  struct Luser *userlptr=0;

  if (ac < 2)
  {
    notice(n_NickServ, lptr->nick, "Syntax: \002DROP\002 nick password");
    notice(n_NickServ, lptr->nick,
      ERR_MORE_INFO,
      n_NickServ,
      "DROP");
  ...
  {
#ifdef EMPOWERADMINS
      if (!IsValidAdmin(lptr) && !IsHelper(utmp))
      {
        notice(n_NickServ, lptr->nick,
          ERR_BAD_PASS);
  ...
  else
  {
    ...
    if (irccmp(dnick, lptr->nick) && FindClient(av[1]))
    {
      notice(n_NickServ, lptr->nick,
        "You may not drop another active nick.",
        dnick);
      return;
    }
    ...
    {
      notice(n_NickServ, lptr->nick,
        "[\002%s\002] has %d nicks linked to it and may not be dropped.",
        dnick, ni->numlinks);
      return;
    }
  ...
  userlptr = FindClient(dnick);

  if ((ni->flags & NS_CLOAK) && (ni->cloak) && (userlptr))
    do_sethost(n_NickServ, userlptr, userlptr->orighost);
...

{
  ...
  {
    struct UserChannel *uc;

    /* For each channel the user is in, if autoflags is set, redo
     * CheckOp to op them if necessary */
    for (uc = lptr->firstchan; uc; uc = uc->next)
      if (uc->redo_cs_checkop)
        cs_CheckOp(uc->chptr, FindChan(uc->chptr->name), lptr->nick);
  }  
  ...
  if ((nptr->flags & NS_CLOAK) && nptr->cloak)
    do_sethost(n_NickServ, lptr, nptr->cloak);
} /* n_identify() */
...

{
  ...
  {
    /* Allow linked nick without password -- jilles */
    curni = FindNick(lptr->nick);
    if (curni != NULL && (curni->flags & NS_IDENTIFIED) && IsLinked(curni, ni))
      goodcoll = 1;
    else if (ac >= 3)
      if (pwmatch(ni->password, av[2]))
        goodcoll = 1;
...

{
  ...
  if (goodcoll)
  {
    ...
    toserv("COLLIDE %s :GHOST command used by %s\n",
      av[1],
      lptr->nick);
...

{
  ...
  if (realptr->lastseen && !online && (!(nptr->flags & NS_HIDEALL) || isadmin || isowner || ishelper))
      ...
      if (nptr != realptr)
	{
	  if (master_online)
	    notice(n_NickServ, lptr->nick,
		   "          Last Seen: %s ago (%s is online)",
		   timeago(realptr->lastseen, 1), nptr->nick); 
	  else
	    {
	      if (nptr->lastseen)
		notice(n_NickServ, lptr->nick,
		       "          Last Seen: %s ago (%s seen %s ago)",
		       timeago(realptr->lastseen, 1), nptr->nick,
		       timeago(nptr->lastseen, 1)); 
	      else
		notice(n_NickServ, lptr->nick,
		       "          Last Seen: %s ago",
  ...
  if (!(nptr->flags & NS_HIDEALL) || isadmin || isowner || ishelper)
  {
    ...
    if (nptr->flags & NS_UNSECURE)
      strcat(buf, "UnSecure, ");
    if (nptr->flags & NS_UNFILTERED)
      strcat(buf, "UnFiltered, ");
  ...
  if (isadmin || isowner || ishelper)
  {
    ...
    /*
     * ChanServDB makes all contact entries the nickname of
     * the LINK master - so make sure we test the contact
     * nickname against the link master's
     */
    if (nptr->ContactChannels)
    {
      ...
      cnt = 0;
      for (tmpchan = nptr->ContactChannels; tmpchan; tmpchan = tmpchan->next)
      {
        notice(n_NickServ, lptr->nick,
          "                     %d) %s",
          ++cnt,
...

{
  struct NickInfo *target, /* target nickname */
                  ...
                  *master; /* master nick, for cloaking purposes */
  int badlink,
      ret;
  ...
  if ((ret = InsertLink(target, nptr)) > 0)
  {
    ...
    master = GetMaster(target);
    if (master && (master->flags & NS_CLOAK) && master->cloak)
      do_sethost(n_NickServ, lptr, master->cloak);
  }
  else if (ret == 0)
  {
    notice(n_NickServ, lptr->nick,
...

{
  ...
  o_Wallops("DROPLINK from %s!%s@%s for nick [%s]",
    lptr->nick, lptr->username, lptr->hostname,
    av[1] );
 ...
 */
static int valid_hostname_remote(const char* hostname)
{
  int         dots  = 0;
  ...
  if (lastslash != NULL && lastslash[1] >= '0' && lastslash[1] <= '9')
                         /*IsDigit(lastslash[1]))*/
    return 0;
  return (0 == dots) ? 0 : 1;
}
...
/*
n_setcloak()
  Change the cloak string for nickname av[1] to av[2]
*/

static void
n_setcloak(struct Luser *lptr, int ac, char **av)

{
  ...
  {
    notice(n_NickServ, lptr->nick,
      "Syntax: \002SETCLOAK <nickname> {ON|OFF} [<hostname>]\002");
    notice(n_NickServ, lptr->nick,
      ERR_MORE_INFO,
      n_NickServ,
      "SETCLOAK");
  ...
  RecordCommand("%s: %s!%s@%s SETCLOAK [%s]...",
    n_NickServ,
    lptr->nick,
    lptr->username,
    lptr->hostname,
  ...
  o_Wallops("SETCLOAK from %s!%s@%s for nick [%s]",
    lptr->nick, lptr->username, lptr->hostname,
    av[1] );
  ...
  userlptr = FindClient(av[1]);
  realptr = GetMaster(nptr);

  if (ac < 3)
  {
    notice(n_NickServ, lptr->nick,
      "Cloaking for %s is [\002%s\002]",
      av[1],
      realptr->flags & NS_CLOAK ? "ON" : "OFF");
    return;
  }

  if(!irccmp(av[2], "ON"))
  {
    realptr->flags |= NS_CLOAK;
  }
  else if(!irccmp(av[2], "OFF"))
  {
    realptr->flags &= ~NS_CLOAK;
  }
  else
  ...
  if(ac < 4)
  {
    if(realptr->cloak && userlptr != NULL)
    {
      if((realptr->flags & NS_CLOAK) && (nptr->flags & NS_IDENTIFIED))
        do_sethost(n_NickServ, userlptr, realptr->cloak);
      else
        do_sethost(n_NickServ, userlptr, userlptr->orighost);
    }

    notice(n_NickServ, lptr->nick,
      "Cloak for [\002%s\002] has been toggled [\002%s\002]",
      av[1],
      realptr->flags & NS_CLOAK ? "ON" : "OFF");
  }
  else
  {
    ...
    {
      if (!valid_hostname_remote(av[3])) {
        notice(n_NickServ, lptr->nick, "Invalid Hostname [\002%s\002]!",
               av[3]);
        return;
      }

      realptr->cloak = MyStrdup(av[3]);
    ...
    notice(n_NickServ, lptr->nick,
      "Cloak for [\002%s\002] has been toggled [\002%s\002] and changed to [\002%s\002]",
      av[1],
      realptr->flags & NS_CLOAK ? "ON" : "OFF",
      av[3]);
  }
...
static void
n_set_unfiltered(struct Luser *lptr, int ac, char **av)

{
  ...
  RecordCommand("%s: %s!%s@%s SET UNFILTERED %s",
    n_NickServ,
    lptr->nick,
    lptr->username,
    lptr->hostname,
  ...
  {
    notice(n_NickServ, lptr->nick,
      "Unfiltered private messages for your nickname are [\002%s\002]",
      (nptr->flags & NS_UNFILTERED) ? "ON" : "OFF");
    return;
  }

  if (!irccmp(av[2], "ON"))
  {
    nptr->flags |= NS_UNFILTERED;
    toserv(":%s MODE %s +6\n", Me.name, lptr->nick);
    notice(n_NickServ, lptr->nick,
      "Toggled UnFiltered [\002ON\002]");
    return;
  }
  ...
  if (!irccmp(av[2], "OFF"))
  {
    ...
    toserv(":%s MODE %s -6\n", Me.name, lptr->nick);
    notice(n_NickServ, lptr->nick,
      "Toggled UnFiltered [\002OFF\002]");
    return;
  }

  /* user gave an unknown param */
  notice(n_NickServ, lptr->nick,
    "Syntax: \002SET UNFILTERED {ON|OFF}\002");
  notice(n_NickServ, lptr->nick,
    ERR_MORE_INFO,
    n_NickServ,
    "SET UNFILTERED");
...
static void
n_exchans(struct Luser *lptr, int ac, char **av)

{
  ...
  if (!(nptr = GetLink(target)))
    return;

  RecordCommand("%s: %s!%s@%s EXTENDCHANS %s %s",
    n_NickServ,
    lptr->nick,
    lptr->username,
    lptr->hostname,
  ...
  {
    notice(n_NickServ, lptr->nick,
      "Extended channel ability for %s (%s) is [\002%s\002]",
      target, nptr->nick,
      (nptr->flags & NS_EXCHANS) ? "ON" : "OFF");
    return;
  }

  if (!irccmp(value, "ON"))
  {
    nptr->flags |= NS_EXCHANS;
    toserv(":%s MODE %s +u\n", Me.name, target);
    notice(n_NickServ, lptr->nick,
      "Toggled ExtendChans for %s [\002ON\002]",
      nptr->nick);
    return;
  }
  ...
  if (!irccmp(av[2], "OFF"))
  {
    ...
    toserv(":%s MODE %s -u\n", Me.name, target);
    notice(n_NickServ, lptr->nick,
      "Toggled ExtendChans for %s [\002OFF\002]",
      nptr->nick);
    return;
  }
  ...
  /* user gave an unknown param */
  notice(n_NickServ, lptr->nick,
    "Syntax: \002EXTENDCHANS <nickname> [ON|OFF]\002");
  notice(n_NickServ, lptr->nick,
    ERR_MORE_INFO,
    n_NickServ,
    "EXTENDCHANS");
