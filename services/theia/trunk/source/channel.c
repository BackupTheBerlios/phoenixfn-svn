 ...
 *
 * $Id: channel.c,v 1.3 2001/11/12 09:50:55 asuffield Exp $
 */
...
/*
HasException()
 Return the first exception matching the given user, if any.
*/

struct Exception *
HasException(struct Channel *cptr, struct Luser *lptr)
{
  char chkstr[MAXLINE], chkstr2[MAXLINE];
  struct Exception *tempe;
  ...
  tempe = cptr->exceptlist;
  while (tempe && !match(tempe->mask, chkstr) && !match(tempe->mask, chkstr2))
    tempe = tempe->next;
