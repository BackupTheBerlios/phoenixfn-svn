 ...
 *
 * $Id: client.c,v 1.3 2001/11/12 09:50:55 asuffield Exp $
 */
...

{
  ...
  int umode;
  unsigned int ii;
  struct NickInfo *realptr=0, *masterptr=0;
    ...
    if (modes[ii] == '6')
      {
        ...
        umode = UMODE_6;
        if (realptr && !IsOperator(user))
          {
            masterptr = GetMaster(realptr);
            if (realptr->flags & NS_IDENTIFIED)
              {
                if (PLUS)
                  {
                    if (!(masterptr->flags & NS_UNFILTERED))
                      {
                        toserv(":%s MODE %s -6\n", Me.name, user->nick);
                        on = 0;
                      }
	  ...
	  if (realptr)
	    {
	      realptr->flags |= NS_IDENTIFIED;
              if ((masterptr->flags & NS_CLOAK) && (masterptr->cloak))
                do_sethost(n_NickServ, user, masterptr->cloak);
	  ...
	  else
	    {
	      ...
	      toserv(":%s MODE %s -e6u\n", Me.name, user->nick);
	      RecordCommand("User %s has +e umode but is not known to me, setting -e", user->nick);
	      umode = 0;
	    }
	}
    if (!umode)
...

{
  ...
  strncpy(tempuser->realip, line[8], IPLEN);
  strncpy(tempuser->realname, line[9] + 1, REALLEN);
  strncpy(tempuser->orighost, line[6], HOSTLEN);
  ...
  tempuser->realip = MyStrdup(line[8]);
  tempuser->realname = MyStrdup(line[9] + 1);
  tempuser->orighost = MyStrdup(line[6]);
  ...
  while (*ch)
  {
    switch (*ch)
    {
      ...
      case 'X':
      {
        tempuser->umodes |= UMODE_CAPX;
        break;
      }
      case 'p':
      {
        tempuser->umodes |= UMODE_P;
        break;
      }
      case 'j':
      {
        tempuser->umodes |= UMODE_J;
        break;
      }
      case 'o':
      case 'O':
	...
	if (realptr)
	  {
	    ...
	    RecordCommand("User %s has +e umode, marking as identified", tempuser->nick);
	  }
	else
	  {
	    /* Is it one of mine? */
		...
		RecordCommand("User %s has +e umode but is not known to me, setting -e", tempuser->nick);
	      }
	  }
      }
      break;
    case '6':
    {
      tempuser->umodes |= UMODE_6;
      break;
    }
    case 'u':
    {
      tempuser->umodes |= UMODE_U;
      break;
    }
    default: break;
    } /* switch (*ch) */
      ...
      if (realptr->flags & NS_IDENTIFIED)
        {
          ...
          else if(!IsOperator(tempuser) && (tempuser->umodes & UMODE_U))
            {
              toserv(":%s MODE %s -u\n", Me.name, tempuser->nick);
              tempuser->umodes &= ~UMODE_U;
            }
...

{
  ...
  #ifdef CHANNELSERVICES
    while (user->contact_channels)
    {
      fnext = user->contact_channels->next;
      RemContact(user, user->contact_channels->cptr);
      user->contact_channels = fnext;
    }
  #endif
