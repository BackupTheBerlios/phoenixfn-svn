 ...
 *
 * $Id: operserv.c,v 1.3 2001/11/12 09:50:55 asuffield Exp $
 */
...
static void o_blockreg(struct Luser *, int, char **, int);
static void o_blockreg_on(struct Luser *, int, char **, int);
static void o_blockreg_off(struct Luser *, int, char **, int);
...
/* sub-commands for OperServ BLOCKREG */
static struct OperCommand blockregcmds[] = {
  { "ON", o_blockreg_on, 0, 0 },
  { "OFF", o_blockreg_off, 0, 0 },
...
static struct UmodeInfo
{
  ...
  { 'k', OPERUMODE_K },     /* see channel kicks and removes */
...
/*
o_Wallops()
 Called when an OperServ command is executed - send a +s server notice
*/
...

{
  if (Rehash() == 0)
  {
    ...
    o_Wallops("REHASH from %s!%s@%s", onick, ouser, ohost);
  }
  else
  {
    ...
    o_Wallops("REHASH from %s!%s@%s (failed)", onick, ouser, ohost);
  }
} /* o_rehash() */
...

{
  ...
  o_Wallops("RESTART from %s!%s@%s", onick, ouser, ohost);
...

{
  ...
  if (ac < 3)
    reason = MyStrdup("<No reason given>");
  else
  ...
  /*o_Wallops("KILL %s [%s]",  let them see the reason as for any other kill
    kptr->nick,
    reason);*/

  toserv(":%s KILL %s :%s!%s (%s (%s@%s))\n",
    n_OperServ, av[1], Me.name, n_OperServ, reason, onick, n_OperServ);
...

{
  ...
  if (!nickjupe)
    {
      if (AddVote(av[1], onick) == -1)
      ...
      else
        {
          ...
          o_Wallops("VOTE:%d/%d JUPE %s from %s",
                    CountVotes(av[1]), JUPEVOTES, av[1], onick);
        }
  ...
  else
  {
    ...
    ircsprintf(sendstr,
      "NICK %s 1 %ld +i %s %s %s %lu :%s\n",
      av[1], (long) CurrTime, JUPED_USERNAME, JUPED_HOSTNAME,
      Me.name, 0xffffffffUL, reason);
    toserv(sendstr);
...

{
  ...
  o_Wallops("OMODE from %s!%s@%s - %s %s",
    onick, ouser, ohost, av[1], omodes);
...

{
  ...
  o_Wallops("SECURE %s from %s!%s@%s",
    av[1], onick, ouser, ohost);
...

{
  ...
  o_Wallops("UNGLINE %s from %s!%s@%s",
    av[1], onick, ouser, ohost);
...

{
  ...
  if (fp == NULL)
  {
    os_notice(lptr, sockfd, "Unable to create replacement config file");
    return;
  }
...

{
  ...
  if (ac < 2)
  {
    os_notice(lptr, sockfd, "Syntax: \002FLOODOFF <nickname>\002");
    return;
  }
  ...
  o_Wallops("FLOODOFF %s from %s!%s@%s",
    fptr->nick, onick, ouser, ohost);
  ...
  if (IsProtected(GetUser(ii, fptr->nick, fptr->username, fptr->hostname)))
  {
    ...
    putlog(LOG2, "%s!%s@%s attempted to FLOODOFF protected user %s",
      onick,
      ouser,
      ohost,
      fptr->nick);
...

{
  ...
  if (target->umodes & UMODE_O)
    strcat(tmp, "o");
  if (target->umodes & UMODE_E)
    strcat(tmp, "e");
  if (target->umodes & UMODE_CAPX)
    strcat(tmp, "X");
  if (target->umodes & UMODE_P)
    strcat(tmp, "p");
  if (target->umodes & UMODE_J)
    strcat(tmp, "j");
...

{
  ...
  if (cptr->modes & MODE_C)
    strcat(modes, "c");
  if (cptr->modes & MODE_G)
    strcat(modes, "g");
  if (cptr->modes & MODE_M)
    strcat(modes, "m");
  if (cptr->modes & MODE_I)
    strcat(modes, "i");
  if (cptr->modes & MODE_CAPF)
    strcat(modes, "F");
  if (cptr->modes & MODE_CAPP)
    strcat(modes, "P");
  if (cptr->modes & MODE_CAPQ)
    strcat(modes, "Q");
  if (cptr->modes & MODE_R)
    strcat(modes, "r");
  if (cptr->modes & MODE_Z)
    strcat(modes, "z");
  if (cptr->limit)
    strcat(modes, "l");
  ...
  if (cptr->forward && *cptr->forward)
    strcat(modes, "f");
  if (cptr->throttle && *cptr->throttle)
    strcat(modes, "J");
...

{
  ...
  o_Wallops("ON from %s!%s@%s", onick, ouser, ohost);
} /* o_on() */
...

{
  ...
  o_Wallops("OFF from %s!%s@%s", onick, ouser, ohost);
} /* o_off() */
...
{
  os_notice(lptr, sockfd,
    "Reload does not currently work.");
#if 0
  int goodreload;
  ...
  if (goodreload)
  {
    ...
    o_Wallops("RELOAD from %s!%s@%s", onick, ouser, ohost);
  }
  else
    os_notice(lptr, sockfd,
...

{
  ...
  o_Wallops("KILLCHAN %s %s[%s] from %s!%s@%s",
    chptr->name,
    argbuf,
    reason,
    onick, ouser, ohost);
...

{
  ...
  o_Wallops("KILLHOST %s [%s] from %s!%s@%s", av[1], reason, onick, ouser, ohost);
...
/*
o_blockreg()
 Turn NickServ REGISTER on or off
*/

static void
o_blockreg(struct Luser *lptr, int ac, char **av, int sockfd)

{
  ...
  {
    os_notice(lptr, sockfd,
      "NickServ REGISTER is \002%s\002",
      (RegisterDisabled) ? "disabled" : "enabled");

    return;
  }

  cptr = GetoCommand(blockregcmds, av[1]);
...
static void
o_blockreg_on(struct Luser *lptr, int ac, char **av, int sockfd)

{
  ...
  if (RegisterDisabled)
  {
    os_notice(lptr, sockfd, "Registrations are currently disallowed");
    return;
  }
  ...
  putlog(LOG1,
    "Blocking new REGISTERs: By %s!%s@%s",
    onick,
    ouser,
    ohost);

  o_Wallops("%s has disabled new REGISTERs",
    onick);

  os_notice(lptr, sockfd, "NickServ REGISTER is now disabled");
} /* o_blockreg_on() */

static void
o_blockreg_off(struct Luser *lptr, int ac, char **av, int sockfd)

{
  ...
  if (!RegisterDisabled)
  {
    os_notice(lptr, sockfd, "Registrations are currently allowed");
    return;
  }
  ...
  putlog(LOG1,
    "Allowing new REGISTERs: By %s!%s@%s",
    onick,
    ouser,
    ohost);

  o_Wallops("%s has re-enabled new REGISTERs",
    onick);

  os_notice(lptr, sockfd, "NickServ REGISTER is now enabled");
} /* o_blockreg_on() */
...
Possible Usermodes:
  ...
  j - see channel joins
  k - see channel kicks and removes
...

{
  ...
  else
  {
    if (cptr->since == 0)
    {
      /* if the channel's TS is 0, we can't join with TS-1, so remove all
         the users */
...

{
  ...
  for (ii = 0; ii < CHANLIST_MAX; ++ii)
  {
    ...
    for (cptr = chanlist[ii]; cptr; cptr = cptr->next)
    {
      ...
      if (cptr->contact)
        csm += strlen(cptr->contact);
      if (cptr->alternate)
        csm += strlen(cptr->alternate);
      if (cptr->topic)
