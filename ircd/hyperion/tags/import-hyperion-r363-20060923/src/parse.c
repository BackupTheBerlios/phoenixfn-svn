                               ...
                               char *pbuffer)
{
  ...
  if (!strchr(psender, '.'))
    ...
    {
      sendto_ops_flag_butone(cptr, UMODE_DEBUG, "Unknown prefix from %s, Squitting %s", 
			     get_client_name(cptr, MASK_IP), psender);
      sendto_one(cptr, ":%s SQUIT %s :(Unknown prefix from %s)",
                 me.name, psender, get_client_name(cptr, MASK_IP));
    }
}
          ...
          */
          if (!IsMe(acptr) && IsPerson(acptr))
            sendto_prefix_one(acptr, sptr,":%s %s %s%s",
                              !MyConnect(acptr) || HasUmode(acptr, UMODE_AUSPEX) ? parv[0] : me.name, numeric, nick, buffer);
