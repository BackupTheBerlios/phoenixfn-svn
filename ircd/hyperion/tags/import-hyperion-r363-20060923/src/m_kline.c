...

{
  ...
  if (strchr(host, ':'))
    {
      escapedhost[0] = '[';
      strncpy_irc(escapedhost + 1, host, HOSTLEN + 1);
      strcat(escapedhost, "]");
    }
  else
    strncpy_irc(escapedhost, host, HOSTLEN + 1);
                ...
                char *parv[])
{
  ...
  if (reason && strchr(reason, ';') && IsServer(sptr))
    {
  ...
  logprintf(L_TRACE, "%s added K-Line for [%s@%s] [%s]",
      rcptr ? rcptr->name : sptr->name, user, host, reason ? reason : "No reason");
...
dline_client(struct Client *acptr, const char *reason)
{
  ...
  ip_mask = 0xFFFFFFFFL;
#ifdef IPV6
  ip_host = ntohl((uint32_t)acptr->ip.S_ADDR);
#else
  ip_host = ntohl(acptr->ip.s_addr);
#endif

  strcpy(host, inetntoa((char*) &acptr->ip));
