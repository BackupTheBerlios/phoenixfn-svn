  ...
  if (parc < 2 || !*awy2)
    {
	 ...
	 commented out this sendto_serv_butone() call -Dianora */
      /* now we can send this .. as long as they were away before
	 away messages can get out of sync, but it's much less load
	 on the network -is */
      /* Let's test this *before* we set sptr->user->away to NULL -- asuffield */
      if (sptr->user->away)
...
/* some lamers scripts continually do a /away, hence making a lot of
   unnecessary traffic. *sigh* so... as comstud has done, I've
   commented out this sendto_serv_butone() call -Dianora */
  /* see my comment above -is */
  if (!sptr->user->away || strcmp(sptr->user->away, awy2)) /* So why not propogate only when it's changed? -- asuffield */
