		  ...
		  if (MyClient(sptr) && !ChannelExists(fchname))
		    {
		      sendto_one(sptr, form_str(ERR_NOSUCHCHANNEL),
				 me.name, sptr->name, "MODE +b redirection channel:");
		      break;
		    }
...
/* 	      struct Channel *fchptr; */
		...
		{
		  sendto_one(sptr, form_str(ERR_NOSUCHCHANNEL),
			     me.name, sptr->name, "MODE +f redirection channel:");
		  break;
		}
          ...
          if( chptr )   /* The channel existed, so I can't join it */
            {
              ...
              if( (chptr->users == 0) && !IsSetOperGod(sptr) )
                {
  ...
  */
  for (i = 0, name = strtoken(&p, parv[1], ","); name;
       name = strtoken(&p, (char *)NULL, ","))
    {
      ...
      if (server_was_split && MyClient(sptr) && !IsSetOperGod(sptr))
        {
              sendto_one(sptr, form_str(ERR_UNAVAILRESOURCE),
                         me.name, parv[0], name);
              continue;
      ...
      if(cold_start && MyClient(sptr) && !IsSetOperGod(sptr))
        {
              sendto_one(sptr, form_str(ERR_UNAVAILRESOURCE),
                         me.name, parv[0], name);
              continue;
...
static  void sjoin_sendit(struct Client *from,
                          ...
                          struct Channel *chptr)
{
  sendto_channel_butserv(chptr, from, ":%s MODE %s %s %s", NETWORK_NAME,
