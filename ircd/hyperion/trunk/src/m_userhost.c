                   ...
                   char *parv[])
{
  ...
  cur_len = ircsnprintf(buf, BUFSIZE, form_str(RPL_USERHOST), me.name, parv[0], "");
  t = buf + cur_len;
      ...
      if ((acptr = find_person(cn, NULL)))
        {
          if (acptr == sptr) /* show real IP for USERHOST on yourself */
            rl = ircsnprintf(response, NICKLEN*2+USERLEN+HOSTLEN+30, "%s%s=%c%s@%s ",
			     acptr->name,
			     HasUmode(acptr,UMODE_OPER) && SeesOpers(sptr) ? "*" : "",
			     (acptr->user->away) ? '-' : '+',
			     acptr->username,
			     acptr->sockhost);
          else
          ...
          if((rl + cur_len) < (BUFSIZE-10))
            {
              ircsnprintf(t, NICKLEN*2+USERLEN+HOSTLEN+30, "%s", response);
              t += rl;
              cur_len += rl;
            }
          else
