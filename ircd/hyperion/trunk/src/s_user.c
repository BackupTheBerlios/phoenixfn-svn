...
/*  forced = (parc >= 2); */
  forced = (CanForceLusers(sptr) && (parc > 3));
  ...
  if (MyConnect(sptr))
    {
      ...
      switch(check_client(sptr,username,&reason))
        ...
        case I_LINE_FULL:
        case I_LINE_FULL2:
          sendto_ops_flag(UMODE_FULL, "I:line is full for %s", sptr->host);
          logprintf(L_INFO,"Too many connections (I:line full) from %s.", get_client_host(sptr));
          ServerStats->is_ref++;
...
/*                     } */
      ...
      if (acptr) /* duplicates ghost check, oh well */
	{
	  ...
	  aconf = find_matching_mtrie_conf(sptr->host, sptr->username, sptr->name, ntohl((unsigned int)sptr->ip.S_ADDR));
   ...
   *  -- asuffield 
   */
  sendto_local_ops_flag(UMODE_CCONN, "Client connecting: %s (%s@%s) [%s] [%s]",
			nick, sptr->username, sptr->host,
			sptr->sockhost, sptr->servptr->name);

  /* This isn't very useful, but is very loud -- asuffield */
#if 0
  if (MyConnect(sptr) && (sptr->username[0] == '~' || isdigit(sptr->host[0])))
    sendto_ops_flag(UMODE_CCONN,
			   "Auth warning on %s (%s@%s) %s%s",
			   sptr->name, sptr->username, sptr->host,
			   (sptr->username[0] == '~') ? "[ident fail] " : "",
			   (isdigit(sptr->host[0])) ? "[dns fail] " : "");
#endif
                   ...
                   char* username, char *host, char *server, char *ip, char *realname)
{
  ...
  if (!MyConnect(sptr))
  ...
  else
    {
      ...
      SetBit(sptr->umodes,UMODE_INVISIBLE);
#endif
