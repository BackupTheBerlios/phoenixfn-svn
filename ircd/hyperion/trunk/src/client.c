          ...
          if(dline_in_progress)
            {
              if(!IsServer(cptr) &&  
#ifdef IPV6
                 (aconf = match_Dline(ntohl((uint32_t)cptr->ip.S_ADDR))) )
#else
                   ...
                   */
                {
                  if(IsConfElined(aconf))
                    {
                      sendto_ops_flag(UMODE_CCONN, "D-line over-ruled for %s, client is E-lined",
                  ...
                  if((aconf = find_kill(cptr))) /* if there is a returned
                                                   struct ConfItem.. then kill it */
                    {
                      if(aconf->status & CONF_ELINE)
                        {
                          sendto_ops_flag(UMODE_CCONN, "K-line over-ruled for %s, client is E-lined",
					  get_client_name(cptr, HIDE_IP));
			  continue;
                        }
      ...
      if (ping < (currenttime - cptr->lasttime))
        {
           ...
           */
          if ((GlobalSetOptions.dopingout || !IsClient(cptr)) &&
  ...
  if (MyConnect(sptr))
    {
      ...
      sptr->flags |= FLAGS_CLOSING;
	      ...
	      sendto_one(sptr, "ERROR :Closing Link: %s (%s)",
			 IsPerson(sptr) ? sptr->host : "127.0.0.1", comment);
            }
          /*
