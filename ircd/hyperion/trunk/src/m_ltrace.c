                ...
                char *parv[])
{
  ...
  if(MyClient(sptr))
    sendto_ops_flag(UMODE_SPY, "ltrace requested by %s (%s@%s) [%s]",
      ...
      if (SeesOpers(sptr) && HasUmode(acptr,UMODE_OPER))
        { 
          sendto_one(sptr, form_str(RPL_TRACEOPERATOR),
                     ...
                     name,
                     HasUmode(sptr,UMODE_AUSPEX) || !IsIPHidden(acptr) ? ip : "255.255.255.255",
                     now - acptr->lasttime,
  ...
  if (dow && LIFESUX && !SendWhileLifeSucks(sptr))
    {
      sendto_one(sptr,form_str(RPL_LOAD2HI),me.name,parv[0],"LTRACE");
      return 0;
    }
   ...
   */
  if (!HasUmode(sptr,UMODE_OPER) || !cnt)
    {
      if (cnt)
          return 0;
