 ...
 *  USA
 *
 *  $Id: m_join.c 1020 2006-03-10 00:16:30Z jilles $
 */
...
DECLARE_MODULE_AV1(join, NULL, NULL, join_clist, join_hlist, NULL, "$Revision: 1020 $");
		...
		char *key)
{
	...
	while (depth < 16)
	{
		...
		if (i == 0)
			return chptr;
		if (i != ERR_INVITEONLYCHAN && i != ERR_NEEDREGGEDNICK && i != ERR_THROTTLE)
			return NULL;
		depth++;
	}
...
m_join(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	for(i = 0, name = strtoken(&p, chanlist, ","); name; name = strtoken(&p, NULL, ","))
	{
	    ...
	    key = (key) ? strtoken(&p2, NULL, ",") : NULL, name = strtoken(&p, NULL, ","))
	{
		...
		{
			if ((i != ERR_NEEDREGGEDNICK && i != ERR_THROTTLE && i != ERR_INVITEONLYCHAN) || ((chptr = check_forward(source_p, chptr, key)) == NULL))
			{
