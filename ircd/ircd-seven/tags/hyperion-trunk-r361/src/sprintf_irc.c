...
/* This writes NO MORE THAN maxlen bytes, and adds a trailing \0 if at all
 * possible.
 * It may return less than maxlen bytes, even if it could have returned more.
 * I do not consider this a bug. If you want to do something crazy, allocate
 *  more memory and grab the whole thing.
 *  -- asuffield
 */

int
vsnprintf_irc(register char *str, register int maxlen, register const char *format,
	      va_list args)
{
  register char c;
  ...
  /* OK, decrement maxlen now so we have a free character to add a \0
   * on the end of every string
   */
  maxlen--;
  ...
  while ((c = *format++))
      ...
      if (c == '%')
	{
	  ...
	  if (c == 's')
	    {
	      register const char *p1 = va_arg(args, char *);
	      if (p1 && (*str = *p1))
		{
		  ++bytes;
		  if (bytes >= maxlen)
	  ...
	  if (c != '%')
	    {
	      ...
	      format -= 2;
	      /* Allow vsnprintf to use the extra byte we saved for \0 */
	      ret = vsnprintf(str, maxlen - bytes + 1, format, args);
	      str += ret;
	      bytes += ret;
...
int
ircsnprintf(register char *str, register int maxlen, const char *format, ...)
{
  va_list args;
  register int bytes;
  ...
  if (strstr(format, "%z"))
    {
      /* This is slow as hell. Don't really have any other options */
      DupString(new_format, format);
      /* Replace every %z with a %l */
      /* XXX: Broken for %%z, but this doesn't bother me much (yet) */
      while ((p = strstr(new_format, "%z")))
	p[1] = 'l';
      bytes = vsnprintf_irc(str, maxlen, new_format, args);
      MyFree(new_format);
    }
  else
#endif
    bytes = vsnprintf_irc(str, maxlen, format, args);
