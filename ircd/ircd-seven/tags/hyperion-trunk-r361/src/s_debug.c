...
/*  size_t rm = 0;        *//* res memory used */
  ...
  sendto_one(cptr, ":%s %d %s z :Client Local %d(%zd) Remote %d(%zd)",
	     me.name, RPL_STATSDEBUG, nick, lc, lcm, rc, rcm);
  sendto_one(cptr, ":%s %d %s z :Users %d(%zd) Invites %d(%zd) Servers %d(%zd)",
	     me.name, RPL_STATSDEBUG, nick, us, us * sizeof(anUser), usi,
	     usi * sizeof(Link), ss, ss * sizeof(struct Server));
  sendto_one(cptr, ":%s %d %s z :User channels %d(%zd) Aways %d(%zd)",
	     me.name, RPL_STATSDEBUG, nick, usc, usc * sizeof(Link), aw, awm);
  sendto_one(cptr, ":%s %d %s z :Attached confs %d(%zd)",
  ...
  sendto_one(cptr, ":%s %d %s z :Message files %zd",
  ...
  sendto_one(cptr, ":%s %d %s z :Hash: client %d(%zd) chan %d(%zd)",
	     me.name, RPL_STATSDEBUG, nick,
	     U_MAX, client_hash_table_size, CH_MAX, channel_hash_table_size);

  sendto_one(cptr, ":%s %d %s z :Dbuf blocks allocated %zd(%zd), used %zd(%zd)",
...
/*   rm = cres_mem(cptr); */
  ...
  sendto_one(cptr, ":%s %d %s z :Total: ww %zd ch %zd cl %zd co %zd db %zd",
  ...
  sendto_one(cptr,
	     ":%s %d %s z :Local client Memory in use: %zd Local client Memory allocated: %zd Overheads: %zd",
  ...
  sendto_one(cptr,
	     ":%s %d %s z :Remote client Memory in use: %zd Remote client Memory allocated: %zd Overheads: %zd",
  ...
  sendto_one(cptr,
	     ":%s %d %s z :anUser Memory in use: %zd anUser Memory allocated: %zd Overheads: %zd",
  ...
  sendto_one(cptr,
	     ":%s %d %s z :Links Memory in use: %zd Links Memory allocated: %zd Overheads: %zd",
  ...
  sendto_one(cptr,
	     ":%s %d %s z :FLUD Memory in use: %zd FLUD Memory allocated: %zd Overheads: %zd",
	     me.name, RPL_STATSDEBUG, nick, flud_memory_used,
	     flud_memory_allocated, flud_memory_overheads);
#endif

  sendto_one(cptr,
	     ":%s %d %s z :TOTAL: %zd Available:  Current max RSS: %zd",
