      ...
      else
        {
          for (acptr = GlobalClientList;
               ...
               acptr = acptr->next)
            {
               ...
               * link --jto
               */
              /* huh? we already checked that above -- jilles */
...
time_t try_connections(time_t currenttime)
{
  ...
  if (burst_in_progress != NULL)
    return CurrentTime + 10;

#ifndef AUTOCONN_AGGRESSIVE
  /* Don't autoconnect if we are trying to connect to something already
   * -- jilles */
...
#ifdef AUTOCONN_AGGRESSIVE
      if (cptr == NULL)
        {
	  /* This may be a bit slow, but connecting/handshake state
	   * servers aren't in any hash tables -- jilles */
	  for (i = 0; i <= highest_fd; i++)
	    {
	      cptr = local[i];
	      if (cptr == NULL)
		continue;
	      if ((IsConnecting(cptr) || IsHandshake(cptr)) && !irccmp(cptr->name, aconf->name))
		break;
	      cptr = NULL;
	    }
        }
...
void show_servers(struct Client *cptr)
{
  ...
  for(cptr2 = serv_cptr_list; cptr2; cptr2 = cptr2->next_server_client)
    {
      ++j;
      duration = CurrentTime - cptr2->firsttime;
      sendto_one(cptr, ":%s %d %s V :%s (%s!%s@%s) Idle: %.1ld SendQ: %d Connected: %d days, %d:%02d:%02d",
                 me.name, RPL_STATSDEBUG, cptr->name, cptr2->name,
                 (cptr2->serv->by[0] ? cptr2->serv->by : "Remote."), 
                 "*", "*", CurrentTime - cptr2->lasttime,
		 (int)DBufLength(&cptr2->sendQ), duration / 86400,
		 (duration % 86400) / 3600, (duration % 3600) / 60,
		 duration % 60);
  ...
  sendto_one(cptr, ":%s %d %s V :%d Server%s", me.name, RPL_STATSDEBUG,
             cptr->name, j, (j==1) ? "" : "s");
}
