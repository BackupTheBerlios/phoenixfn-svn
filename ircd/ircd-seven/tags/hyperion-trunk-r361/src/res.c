...
static void start_resolver(void)
{
  ...
  if (ResolverFileDescriptor < 0) {
    ...
    mangle_socket_generic(ResolverFileDescriptor);
...
/*
 * flush_cache - tries to remove as many entries from the cache
 * as possible
 */
static void flush_cache(void)
{
...
static time_t expire_cache(time_t now)
{
  ...
  /* Removals could fail and leave the entry in the list;
   * we want to take those into account as well -- jilles */
  for (cp = cacheTop; cp; cp = cp->list_next)
    if (next > cp->expireat)
      next = cp->expireat;
  /* Don't check more often than once per ten seconds */
  if (next < now + 10)
    next = now + 10;
  return next;
}
...
static aCache* make_cache(ResRQ* request)
{
  ...
  if (nextCacheExpire > cp->expireat)
    nextCacheExpire = cp->expireat;
#ifdef DEBUG
...
int m_dns(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if (parv[1] && *parv[1] == 'l') {
    for(cp = cacheTop; cp; cp = cp->list_next) {
      hp = &cp->he.h;
      sendto_one(sptr, "NOTICE %s :Ex %.1ld refs %.1d ttl %.1ld host %s(%s)",
                 parv[0], cp->expireat - CurrentTime, cp->reply.ref_count, cp->ttl,
...
unsigned long cres_mem(aClient *sptr)
{
  ...
  sendto_one(sptr, ":%s %d %s z :RES table %.1ld",
             me.name, RPL_STATSDEBUG, sptr->name, ts);
  sendto_one(sptr, ":%s %d %s z :Structs %.1ld IP storage %.1ld Name storage %.1ld",
