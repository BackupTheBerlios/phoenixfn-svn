...
int m_trace(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
  ...
  if (!HasUmode(sptr,UMODE_AUSPEX))
    {
      if (parv[1] && !strchr(parv[1],'.') && (strchr(parv[1], '*')
					      || strchr(parv[1], '?'))) /* bzzzt, no wildcard nicks for non(auspex)opers */
        {
  ...
  if(MyClient(sptr))
    sendto_ops_flag(UMODE_SPY, "trace requested by %s (%s@%s) [%s]",
  ...
  if(!HasUmode(sptr,UMODE_AUSPEX) || !dow) /* non-oper traces must be full nicks */
    {
      const char* name;
      const char* ip;
      int         c_class;
      ...
      if (SeesOpers(sptr) && HasUmode(acptr,UMODE_OPER))
        {
          sendto_one(sptr, form_str(RPL_TRACEOPERATOR),
                     ...
                     "255.255.255.255",
#else
                     SeesAllConnections(sptr)?ip:(IsIPHidden(acptr)?"255.255.255.255":ip),
#endif
                     now - acptr->lasttime,
  ...
  if (dow && LIFESUX && !SendWhileLifeSucks(sptr))
    {
      sendto_one(sptr,form_str(RPL_LOAD2HI),me.name,parv[0],"TRACE");
      return 0;
    }
...
#else
	  if (IsPerson(acptr) &&
	      (!IsInvisible(acptr) || SeesAllConnections(sptr)))
	    {
	      link_u[acptr->from->fd]++;
	    }
#endif
  ...
  for (i = 0; i <= highest_fd; i++)
    {
	   ...
	   *
	   * Or at least, those who SeesOpers() -- asuffield
           */
          if ((HasUmode(sptr,UMODE_AUSPEX) &&
	       (MyClient(sptr) || !(dow && IsInvisible(acptr))))
              || !dow || (HasUmode(acptr,UMODE_OPER) && SeesOpers(sptr)))
            {
              if (HasUmode(acptr,UMODE_OPER))
                sendto_one(sptr,
                           ...
                           "255.255.255.255",
#else
                           IsSetOperAuspex(sptr)?ip:(IsIPHidden(acptr)?"255.255.255.255":ip),
#endif
                           now - acptr->lasttime,
                           (acptr->user)?(now - acptr->user->last):0);
              else
  ...
  /* Hack -- show unknowns pending ident/dns -- jilles */
  if (doall && HasUmode(sptr,UMODE_AUSPEX))
    {
      const char* name;
      const char* ip;
      int         c_class;
