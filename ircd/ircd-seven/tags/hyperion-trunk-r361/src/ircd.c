...
time_t  cr_challengeexpirytime = 0;
#endif
 ...
 */
static void bad_command(void) noreturn_attribute;
...
static void parse_command_line(int argc, char* argv[])
{
  ...
  while ((opt = getopt(argc, argv, options)) != EOF) {
    ...
    case 'h':
      if (optarg)
        strncpy_irc(me.name, optarg, HOSTLEN + 1);
      break;
    case 'l':
      if (optarg)
...
#ifndef ZIP_LINKS
      printf("ircd %s(%s)\n\tcompiled %s\n\tzlib %s\n\tircd_dir: %s\n",
	version, serial, creation, "not used",
	ConfigFileEntry.dpath);
#else
      printf("ircd %s(%s)\n\tcompiled %s\n\tzlib %s\n\tircd_dir: %s\n",
	version, serial, creation, zlib_version,
	ConfigFileEntry.dpath);
#endif
      exit(0);
    case 'x':
...
static time_t io_loop(time_t delay)
{
  ...
  if ((CurrentTime = time(NULL)) == -1)
    {
      logprintf(L_ERROR, "Clock Failure (%d)", errno);
      sendto_ops_flag(UMODE_SERVNOTICE, "Clock Failure (%d), TS can be corrupted", errno);
      restart("Clock Failure");
    }

  if (CurrentTime < lasttimeofday)
    {
      ircsnprintf(to_send, 200, "System clock is running backwards - (%d < %d)",
              ...
              if (NOISYHTM)
                {
                  sprintf(to_send, 
                        "Entering high-traffic mode - (%.1fk/s > %dk/s)",
                                (float)currlife, LRV);
                  sendto_ops_flag(UMODE_SERVNOTICE, "%s", to_send);
                }
            }
          else
            {
          ...
          if (LIFESUX)
            {
              LIFESUX = 0;
              if (NOISYHTM)
                sendto_ops_flag(UMODE_SERVNOTICE, "Resuming standard operation . . . .");
            }
        }
  ...
  */
   ...
   */
  {
    static time_t last_check_all_time=0;
#ifdef CLIENT_SERVER
    if (!LIFESUX || (last_check_all_time + LIFESUX) < CurrentTime)
      {
#else
    if ((last_check_all_time + (LIFESUX + 1)) < CurrentTime)
      {
#endif
        read_message(delay, FDL_ALL); /*  check everything! */
        last_check_all_time = CurrentTime;
      }
   }
#else
  read_message(delay, FDL_ALL); /*  check everything! */
...
static void initialize_global_set_options(void)
{
 ...
 strcpy(GlobalSetOptions.noidentd_prefix, "~");
...
static void initialize_message_files(void)
  {
  InitMessageFile( HELP_MOTD, oper_help_file, &ConfigFileEntry.helpfile );
 ...
 * inputs       - none
 * output       - none
 * side effects - write the pid of the ircd to pid_file
 */
...
static void write_pidfile(void)
{
  ...
  if ((fd = open(pid_file, O_CREAT|O_WRONLY, 0600))>=0)
    {
      ircsnprintf(buff, 20, "%d\n", (int)getpid());
      if (write(fd, buff, strlen(buff)) == -1)
        logprintf(L_ERROR,"Error writing to pid file %s", pid_file);
      close(fd);
      return;
    }
  else
    logprintf(L_ERROR, "Error opening pid file %s", pid_file);
}
...
void write_stats(void)
{
  int fd;
  char buff[20];
  if ((fd = open(max_client_file, O_CREAT|O_WRONLY, 0600))>=0)
    {
      ircsnprintf(buff, 20, "%d\n%d\n", Count.max_loc, Count.max_tot);
      if (write(fd, buff, strlen(buff)) == -1)
        logprintf(L_ERROR,"Error writing to max clients file %s", max_client_file);
      close(fd);
      return;
    }
  else
    logprintf(L_ERROR, "Error opening max clients file %s", max_client_file);
}
#endif /* SAVE_MAXCLIENT */
...
int main(int argc, char *argv[])
{
  ...
  srandom((unsigned int)((time(0) ^ gethostid()) % getpid()));
  ServerRunning = 0;
 ...
 */
  ...
  ConfigFileEntry.dpath = work_dir;

  ConfigFileEntry.configfile = config_file;
  ConfigFileEntry.klinefile = kline_config_file;
  ConfigFileEntry.dlinefile = dline_config_file;
...
# endif 
  ...
  strncpy_irc(me.host, aconf->host, HOSTLEN + 1);
...
#ifdef SAVE_MAXCLIENT
  if ((file = fbopen(max_client_file, "r")))
    {
  ...
  logprintf(L_NOTICE, "Server Ready");
