...
/* Count of the number of connected servers using this class
 * Something is wrong with using clptr->links for this, and I
 *  don't know what, so work around it
 */
int servers_connected_in_class[512];
		  ...
		  int maxglobalip)
{
  ...
  if ((t == ClassList) && (class != 0))
    {
      expect_malloc;
      p = (aClass *)make_class();
      malloc_log("add_class() allocating class %d (%zd bytes) at %p",
                 class, sizeof(struct Class), (void *)p);
      p->next = t->next;
      t->next = p;
    }
  else
  ...
  MaxLocalIP(p) = maxlocalip;
  MaxGlobalIP(p) = maxglobalip;
  if (p != t)
    Links(p) = 0;
}
...
void    initclass()
{
  ...
  malloc_log("initclass() allocating class 0 (%zd bytes) at %p",
             sizeof(struct Class), (void *)ClassList);
  ...
  ClassList->next = NULL;
  for (i = 0; i < 512; i++)
    servers_connected_in_class[i] = 0;
}
...
void    report_classes(aClient *sptr)
{
  ...
  for (cltmp = ClassList; cltmp; cltmp = cltmp->next)
    sendto_one(sptr, form_str(RPL_STATSYLINE), me.name, sptr->name,
	       ...
	       0, MaxGlobalIP(cltmp),
#else
	       MaxLocalIP(cltmp), MaxLocalIP(cltmp),
	       MaxGlobalIP(cltmp), MaxGlobalIP(cltmp),
#endif
	       Links(cltmp));
}
