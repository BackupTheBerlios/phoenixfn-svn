...
** ported to hyperion Nov 7th 2005 -- jilles
**
**      parv[0] = sender nick
...
*/
  ...
  while(fbgets(buf, sizeof(buf), in))
    {
      /* lower case d is exemption from dlines ;-) -- jilles */
      if (buf[1] == conf_separator && buf[0] == 'D')
        {
          /* its a D: line */
          char *found_cidr;
          ...
          if((p = strchr(found_cidr, conf_separator))  == NULL)
            {
...
*/
          ...
          if((p = strchr(buff, conf_separator)) == NULL)
            {
              if (flush_write(sptr, in, out, buf, temppath) < 0)
		return 0;
	      continue;
  ...
  sendto_one(sptr, ":%s NOTICE %s :D-Line for [%s] is removed",
             me.name, parv[0], cidr);
  sendto_ops_flag(UMODE_SERVNOTICE, "%s has removed the D-Line for: [%s]",
             parv[0], cidr);

  logprintf(L_NOTICE, "%s removed D-Line for [%s]", parv[0], cidr);
  return 0;
}
