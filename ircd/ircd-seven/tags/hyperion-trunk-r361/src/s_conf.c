...
static void     initconf(FBFILE*, int, int);
  ...
  if (IsGotId(cptr))
    {
      ...
      strncpy_irc(new_username + strlen(GlobalSetOptions.identd_prefix), username, sizeof(new_username) - strlen(GlobalSetOptions.identd_prefix));
  ...
  else
    {
      ...
      strncpy_irc(new_username + strlen(GlobalSetOptions.noidentd_prefix), username, sizeof(new_username) - strlen(GlobalSetOptions.noidentd_prefix));
      new_username[USERLEN] = '\0';
      aconf = find_matching_mtrie_conf(cptr->sockhost, new_username, nick,
                                       get_ipv4_ip(&cptr->ip));
      /* Keep the IP-based aconf if it is not an I:line match, but
	  ...
	  if (IsConfDoSpoofIp(aconf))
	    strncpy_irc(cptr->spoofhost, aconf->name, HOSTLEN + 1);
	      ...
	      if (!IsConfElined(aconf))
	        {
		  /* let's also check if the spoof is klined :) -- jilles */
	  	  struct ConfItem *aconf2;

		  aconf2 = find_matching_mtrie_conf(cptr->spoofhost,
		                                    new_username, nick,
						    get_ipv4_ip(&cptr->ip));
		  if (aconf2 != NULL && aconf2->status & CONF_KILL)
		    {
		      /* show spoof in closing link message, but do not
		       * send a spoof notice */
		      strncpy_irc(cptr->host, cptr->spoofhost, HOSTLEN + 1);
		      *preason = aconf2->passwd;
		      return (-5);
		    }
	        }
#ifdef SPOOF_NOTICE
              sendto_ops_flag(UMODE_CCONN, "%s spoofing %s(%s) as %s from I:line", cptr->name,
			      cptr->host, cptr->sockhost, cptr->spoofhost);
#endif /* SPOOF_NOTICE */
              strncpy_irc(cptr->host, cptr->spoofhost, HOSTLEN + 1);
              SetIPHidden(cptr);
            }
...
#endif
{
  ...
  ip_found->count_local++;

  /* only check it if its non zero */
  if (ConfMaxLocalIP(aconf) && ip_found->count_local > ConfMaxLocalIP(aconf))
    {
      if(!IsConfFlined(aconf))
        ...
        {
          sendto_one(cptr,
       ":%s NOTICE %s :*** Too many local clients per IP, but you have an >I: line!",
                     me.name,cptr->name);
        }
    }
  if (ConfMaxGlobalIP(aconf) &&
	ip_found->count_local + ip_found->count_remote > ConfMaxGlobalIP(aconf))
    {
      if(!IsConfFlined(aconf))
        return -6; /* Already at maximum allowed ip#'s */
      else
        {
          sendto_one(cptr,
       ":%s NOTICE %s :*** Too many global clients per IP, but you have an >I: line!",
                     me.name,cptr->name);
        }
    }
  ...
  if ( (ptr = ip_hash_table[hash_index]) != (IP_ENTRY *)NULL )
    {
      ...
      newptr->count_local = 0;
      newptr->count_remote = 0;
      newptr->next = ptr;
#ifdef LIMIT_UH
...
void
add_remote_client_ip(struct Client *cptr)
{
  IP_ENTRY *ip_found;

#ifndef IPV6
  /* If we don't have ipv6 compiled in, we can't track ipv6 clients
   * -- jilles */
  if (strchr(cptr->sockhost, ':'))
    return;
#endif
...
#ifdef LIMIT_UH
  ip_found = find_or_add_ip(cptr,cptr->username);
#else
  ip_found = find_or_add_ip(cptr);
#endif

  if (cptr->flags & FLAGS_IPHASH)
  {
    sendto_ops_flag(UMODE_DEBUG, "WTF: tried to add_remote_client_ip() client %s already in the IP hash", cptr->name);
    return;
  }
  cptr->flags |= FLAGS_IPHASH;
  ip_found->count_remote++;
}
...
#ifdef LIMIT_UH
void remove_one_ip(aClient *cptr, int isremote)
#else
void remove_one_ip(struct IN_ADDR *ip_in, int isremote)
#endif
{
  int hash_index;
  IP_ENTRY *last_ptr;
      ...
      if(!memcmp(&ptr->ip, ip_in, sizeof(struct IN_ADDR)))
        {
          if(isremote)
            {
              if(ptr->count_remote != 0)
                ptr->count_remote--;
              else
                sendto_ops_flag(UMODE_DEBUG, "WTF: tried to decrement remote in remove_one_ip() (%d,%d)", ptr->count_local, ptr->count_remote);
            }
          else
            {
              if(ptr->count_local != 0)
                ptr->count_local--;
              else
                sendto_ops_flag(UMODE_DEBUG, "WTF: tried to decrement local in remove_one_ip() (%d,%d)", ptr->count_local, ptr->count_remote);
            }
#ifdef LIMIT_UH
...
#endif
          if(ptr->count_local == 0 && ptr->count_remote == 0)
            {
 ...
 * output - NONE
 * side effects - client is noticed number of connections from their IP
 */
void show_ip_info(struct Client *cptr)
{
	IP_ENTRY *ptr;

	if (!(cptr->flags & FLAGS_IPHASH))
	{
		sendto_ops_flag(UMODE_DEBUG, "show_ip_info(): client %s not in IP hash", cptr->name);
		return;
	}

#ifdef LIMIT_UH
	ptr = find_or_add_ip(cptr,cptr->username);
#else
	ptr = find_or_add_ip(cptr);
#endif
	if (ptr == NULL)
		return;
	sendto_one(cptr, ":%s NOTICE %s :on %d,%d", me.name, cptr->name,
			ptr->count_local, ptr->count_remote);
}
...
static int hash_ip(struct IN_ADDR *ip)
{
  ...
  for (i = 0; i < 4; i++)
	...
	ip->s6_addr[i * 4 + 2] << 8) +
	ip->s6_addr[i * 4 + 3];
      hash += (a >> IP_HASH_BITS) + a;
    }
  hash &= IP_HASH_SIZE-1;
#else
  a = ntohl(ip->s_addr);
  hash = ((a >> IP_HASH_BITS) + a) & (IP_HASH_SIZE-1);
#endif
  return(hash);
}
      ...
      if ((aconf->status & CONF_CLIENT) &&
...
#endif
        {
          if (!IsConfFlined(aconf))
            ...
            {
              sendto_one(cptr, ":%s NOTICE %s :*** Class is full, but you have an >I: line!\n",
                   me.name, cptr->name);
              SetFlined(cptr);
            }
...
static void initconf(FBFILE* file, int use_include, int initsep)
{
  static char  quotes[] = {
    0,    /*  */
    0,    /* a */
  ...
  conf_separator = initsep; /* reinitialize for each file */
  ...
  while (fbgets(line, sizeof(line), file))
    {
      ...
      if (line[0] == '.' && !ircncmp(line + 1, "sep ", 4))
	{
	  /* .sep % */
	  conf_separator = line[5];
	  continue;
	}
      else if(use_include && (line[0] == '.'))
        {
          char *filename;
          char *back;
	...
	case 'f':
	  ...
	  logprintf(L_WARN, "Locops are not supported by this server");
        ...
        case 'y':
          ...
          sendq = maxlocalip = maxglobalip = 0;
          break;
	...
	case '@': /* line that checks if admin has read conf */
	  ...
	  else if (aconf->status & CONF_CLASS)
	    {
              if ((tmp = getfield(NULL)) == NULL)
                break;
              maxlocalip = atoi(tmp);
...
struct ConfItem *find_kill(aClient* cptr)
{
  ...
  if (!aconf)
    aconf = find_is_klined(cptr->host, cptr->username,
			   cptr->name, get_ipv4_ip(&cptr->ip));
  /* If client is e-lined, then its not k-linable */
  /* no, don't do this, check for e-lined in what calls this so we
   * can generate a server notice -- jilles */
  return aconf;
}
...
struct ConfItem *find_is_klined(const char* host, const char *username, const char *nick, unsigned long ip)
{
  ...
  found_aconf = find_matching_mtrie_conf(host, username, nick, ip);
...
static char *getfield(char *newline)
{
  ...
  field = line;
  if ((end = strchr(line,conf_separator)) == NULL)
    {
      line = (char *)NULL;
      if ((end = strchr(field,'\n')) == (char *)NULL)
