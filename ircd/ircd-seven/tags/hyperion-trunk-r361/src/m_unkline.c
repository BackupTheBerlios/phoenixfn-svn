...
process_unkline(const char *user, const char *host, time_t placed, int cache, int *found)
{
  ...
  while(fbgets(buf, sizeof(buf), in)) 
    {
      if((buf[1] == conf_separator) && ((buf[0] == 'k') || (buf[0] == 'K')))
        {
          /* its a K: line */
          char *found_host;
          char *found_user;
	  ...
	  p = strchr(found_host, conf_separator);
          if(p == NULL)
            {
          ...
          *p = '\0';
          ...
          found_comment = p;
          p = strchr(found_comment,conf_separator);
          if(p == (char *)NULL)
            {
	  ...
	  /* If I have more fields, clip here */
          p = strchr(found_user,conf_separator);
	  if (p)
	    {
	      int i;
	      *p = '\0';
	      /* And go find the time it was placed */
	      for (i=0; p && p[0] && (i < 2); i++, p = strchr(p+1, conf_separator));
	      if (p && p[0] && p[1])
		{
		  found_until = atoi(p+1);
		  p = strchr(p+1, conf_separator);
		  if (p && p[0])
		    found_when = atoi(p+1);
		}
	    }
...

	  {
	    ...
	    /* only say it's unklined if it was this kline and either
	     * perm or not expired yet
	     * -- jilles */
	    else if (found != NULL && match &&
			    (found_until == 0 || found_until >= CurrentTime))
	      *found = 1;
	  }
        }                               
      else if(buf[0] == '#')
      ...
      else if (!ircncmp(buf, ".sep ", 5))
	{
	  conf_separator = buf[5];
	  if (!flush_write(out, buf, temppath))
	    return 0;
	}
...
unkline_conf(struct ConfItem *aconf)
{
  ...
  if (aconf->hold == 1)
    {
      sendto_local_ops_flag(UMODE_SERVNOTICE, "Rehashing to finalize unkline of %s@%s", aconf->user, aconf->host);
      return 1;
    }
...
{
  /* already did this recently, don't do it again now -- jilles */
  if (last_rewrite + EXPIRED_KLINE_DELAY > CurrentTime)
    return 1;
...
m_unkline (aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  /* First try to find this exact kline */
  for (aconf = kline_list; aconf; aconf = aconf->kline_next)
    if (!irccmp(aconf->user, user) && !irccmp(aconf->host, host))
      break;
  if (aconf != NULL)
    {
      if (aconf->hold != 0 && aconf->hold < CurrentTime)
        {
          if (!IsServer(cptr) && !IsMe(cptr))
            sendto_one(sptr, ":%s NOTICE %s :K-Line for %s@%s is already %s",
                       me.name, parv[0], user, host,
            	   aconf->hold == 1 ? "removed" : "expired");
          return 0;
        }
    }
  else
    {
      /* Perhaps it's covered by a broader kline */
      ip = 0L;
      (void)is_address(host,&ip,&host_mask);
      aconf = is_klined(host, user, ip);
      if (aconf == NULL)
        {
          if (!IsServer(cptr) && !IsMe(cptr))
      ...
      if (irccmp(aconf->user, user) || irccmp(aconf->host, host))
        {
          /* There is a broader kline than the given user@host, but perhaps
           * there is an exact match in kline.conf. Continue to
           * process_unkline() but do not mark the kline as removed.
           * -- jilles */
          aconf = NULL;
        }
    }
  ...
  if(!found)
    {
      if (!IsServer(cptr) && !IsMe(cptr))
        sendto_one(sptr, ":%s NOTICE %s :No K-Line found for %s@%s",
                   me.name, parv[0],user,host);
      return 0;
    }
