...
aConfItem *trim_ip_Klines(aConfItem *, unsigned int);
aConfItem *trim_ip_Elines(aConfItem *, unsigned int);
                                  ...
                                  struct ip_subtree *right)
{
  ...
  malloc_log("new_ip_subtree(), allocating struct ip_subtree (%zd bytes) at %p",
             sizeof(struct ip_subtree), (void *)temp);
...
static size_t count_ip_subtree(struct ip_subtree *tree)
{
  if (!tree)
    return 0;
  else
    return count_ip_subtree(tree->left) + 
      sizeof(struct ip_subtree) + 
      count_conf_list(tree->conf) + 
      count_ip_subtree(tree->right);
}
...
size_t count_dlines()
{
  ...
  int i;
  for (i = 0; i < 256; i++)
    tot += count_ip_subtree(Dline[i]);
  tot += count_conf_list(leftover);
  return tot;
}

size_t count_ip_klines()
{
  size_t tot = 0;
  int i;
  for (i = 0; i < 256; i++)
    tot += count_ip_subtree(ip_Kline[i]);
  return tot;
}
 ...
 * -good
 */
aConfItem *trim_ip_Klines(aConfItem *cl, unsigned int mask)
{
  aConfItem *temp;
  if (!cl) return NULL;
  if ((cl->flags & CONF_FLAGS_E_LINED))
 ...
 * -good
 */
aConfItem *trim_ip_Elines(aConfItem *cl, unsigned int mask)
{
  aConfItem *temp;
  if (!cl) return NULL;
...
/*
 * find_Dline_exception - match an IP against all unplaced d-line exceptions 
 * -good
 */
aConfItem *find_Dline_exception(unsigned long ip)
{
...
void add_Dline(aConfItem *conf_ptr)
{
  ...
  /* this could hit all ipv6 clients, silently skip it -- jilles */
  if (host_ip == 0xFFFFFFFFUL && host_mask == 0xFFFFFFFFUL)
    return;
  ...
  if(find_Dline_exception(host_ip))  /* it is!  throw it away! */
     return;
...
void add_ip_Kline(aConfItem *conf_ptr)
{
  ...
  if( conf_ptr->ip_mask == 0UL )
    {
      conf_ptr->ip_mask = 0xFFFFFFFFUL;
    }
