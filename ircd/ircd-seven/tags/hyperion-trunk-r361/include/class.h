...
#define MaxLocalIP(x)   ((x)->maxLocalIP)
#define MaxGlobalIP(x)  ((x)->maxGlobalIP)
...
#define ConfMaxLocalIP(x) (ClassPtr(x)->maxLocalIP)
#define ConfMaxGlobalIP(x) (ClassPtr(x)->maxGlobalIP)

extern struct Class* ClassList;  /* GLOBAL - class list */
extern int servers_connected_in_class[];
...
extern  void    add_class(int, int, int, int, long, int, int);
