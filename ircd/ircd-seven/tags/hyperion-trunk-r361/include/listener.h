...
struct Listener {
  ...
  int              server_port;        /* servers can connect here */
  int              user_port;          /* users can connect here */
  int              hidden_port;        /* don't show in /stats P */
};

extern struct Listener* ListenerPollList; /* GLOBAL - listener list */

extern void        accept_connection(struct Listener* listener);
extern void        add_listener(int port, const char* vaddr_ip, int is_server, int is_user, int is_hidden);
